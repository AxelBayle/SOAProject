...
public class accesRessource {
	...
	public void unlock() {
		...
		Response response=client.target("http://localhost:8080/PorteWS/webapi/portes/1").request().post(Entity.entity("", MediaType.APPLICATION_JSON));
	...
	public void lock() {
		Client client= ClientBuilder.newClient();
		Response response=client.target("http://localhost:8080/PorteWS/webapi/portes/0").request().post(Entity.entity("", MediaType.APPLICATION_JSON));
